<template>
    <div class="SidebarPipelinet px-3 p-2">
        ...
        <div v-for="(form, index) in pipelineContent" :key="index" class="pt-2 flex flex-col gap-2">
            <p
            ...
            <div class="border-l-2 ml-4 px-5">
                <div class="flex justify-start items-center gap-2  hover:bg-bg-300 cursor-pointer rounded-xl px-3" v-for="(item, idx) in form.items"
                    :key="idx">
                    <i :class="`fa-regular ${item.icon} fa-lg`" style="color: #71c4ef;"></i>
                    <p
...
import { } from "vue"
import { pipelineContent } from '../constant/pipelineContent';
</script>
