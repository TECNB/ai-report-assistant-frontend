    ...
    <div class="flex p-2 rounded-lg h-5/6">
      ...
      <div class="w-1/4 bg-white rounded-lg p-5 shadow-md mr-2 h-full">
        <div class="relative mb-4">
          <svg class="absolute left-2 top-3 text-gray-400 w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
          </svg>
          <input
              type="text"
              placeholder="请搜索"
              class="border border-gray-300 p-2 pl-8 w-full rounded"
          />
        </div>
      ...
      <div class="w-3/4 bg-white rounded-lg p-1 shadow-md h-full">
        ...
        <div v-if="selectedDatabase" class="w-1/2 h-full bg-white rounded-lg p-5 shadow-md">
          ...
          <div class="mb-5 flex items-center">
            <label class="block text-gray-700 font-bold mr-4 whitespace-nowrap w-1/4 text-right" for="sourceType">
              数据源类型
            </label>
            <el-select id="sourceType" v-model="selectedSource" placeholder="请选择数据源类型" class="flex-grow w-48">
          ...
          <!-- 连接方式 -->
          <div class="mb-5 flex items-center">
            <label class="block text-gray-700 font-bold mr-4 whitespace-nowrap w-1/5 text-right" for="connectionMethod">
              连接方式
            </label>
            <div class="flex-grow">
...
import { ref } from 'vue';
import { dataSourceTypes } from '../constant/DataSourceData.ts';  // 引入数据
