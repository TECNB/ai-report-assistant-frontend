<template>
  <div>
    ...
    <!-- 弹窗界面 -->
    <div class="DataSidebar bg-white text-black fixed inset-y-0 right-0 w-2/5 z-50 p-6 overflow-y-auto" v-if="props.ifShow">
      ...
      <!-- 顶部区域 -->
            ...
            <div class="header-top flex justify-between items-center">
              ...
              <button @click="toggleVisibility" class="text-gray-500 hover:text-gray-700">
            ...
            <div class="header-main">
              ...
              <div class="flex items-center mt-2">
                ...
                <img src="/src/assets/images/icon.png" alt="头像" class="w-6 h-6 rounded-full border-2 border-black" />
                ...
                <button class="bg-gray-800 text-white font-bold py-1 px-5 rounded ml-auto">提问</button> <!-- 添加 ml-4 来调整按钮与前面元素的距离 -->
              </div>
            </div>
      ...
      <div class="quick-question mt-6">
        ...
        <div class="question-list flex flex-wrap gap-4 mt-4">
          <button
              v-for="(item, index) in quickQuestionsData"
              :key="index"
              class="question-btn bg-gray-100 hover:bg-gray-300 text-black py-2 px-4 rounded flex items-center space-x-2"
          >
      ...
      <div class="tab-selection mt-8 flex items-center">
        <div class="relative inline-block">
          <!-- 字段预览 -->
          <button
              @click="activeTab = 'fieldPreview'"
              :class="{'text-gray-500': activeTab === 'fieldPreview'}"
              class="font-semibold text-left pb-2 mr-5"
          >
            字段预览
          </button>

          <!-- 蓝色横杠 -->
          <div v-if="activeTab === 'fieldPreview'" class="absolute left-1/4 right-2/4 bottom-0 h-1 bg-blue-500"></div> <!-- 缩小横杠长度 -->
        </div>

        <div class="relative inline-block">
          <!-- 数据预览 -->
          <button
              @click="activeTab = 'dataPreview'"
              :class="{'text-gray-500': activeTab === 'dataPreview'}"
              class="font-semibold text-left pb-2"
          >
            数据预览
          </button>

          <!-- 蓝色横杠 -->
          <div v-if="activeTab === 'dataPreview'" class="absolute left-1/4 right-1/4 bottom-0 h-1 bg-blue-500"></div> <!-- 缩小横杠长度 -->
        </div>
        ...
        <div class="ml-auto">
          <div class="relative">
            <input
                type="text"
                placeholder="搜索字段"
                class="search-input w-32 p-0.5 bg-write border border-gray-300 text-black rounded-full pl-10 shadow-sm"
            />
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 4a7 7 0 100 14 7 7 0 000-14zm0 0l6 6" /> <!-- 替换为搜索图标 -->
            </svg>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'fieldPreview'" class="field-preview mt-4">
        ...
        <!-- 数据行 -->
        <div v-for="(field, index) in fieldPreviewData" :key="index" class="field-row flex py-2 border-gray-300">
